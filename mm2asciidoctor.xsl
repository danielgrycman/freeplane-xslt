...
<!--
    MINDMAPEXPORTFILTER adoc;asciidoc Asciidoctor
    (c) by Daniel Grycman, 2015
...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="headingIndicator">
    ...
    <xsl:if test="$count > 0 and $count = 1">
      <xsl:text>==</xsl:text>
      <!--<xsl:call-template name="headingIndicator">
        <xsl:with-param name="count" select="$count - 1"/>
      </xsl:call-template>-->
    </xsl:if>
    <xsl:if test="$count > 0 and $count = 2">
      <xsl:text>===</xsl:text>
    </xsl:if>
    <xsl:if test="$count > 0 and $count = 3">
      <xsl:text>====</xsl:text>
    </xsl:if>
    <xsl:if test="$count > 0 and $count = 4">
      <xsl:text>=====</xsl:text>
    </xsl:if>
    <xsl:if test="$count > 0 and $count = 5">
      <xsl:text>======</xsl:text>
    </xsl:if>
  </xsl:template>
